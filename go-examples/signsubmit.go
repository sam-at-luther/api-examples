...
// REQUIRES SOME SETUP OF TOKEN AND URLS

// Algorand Berlin Hackathon
// const algodAddress = "http://berlinhack.algodev.network:9100"
...
func main() {
	...
	for _, wallet := range listResponse.Wallets {
		...
		if wallet.Name == "integration" {
			fmt.Printf("found wallet '%s' with ID: %s\n", wallet.Name, wallet.ID)
			exampleWalletID = wallet.ID
		}
	}
	// Get a wallet handle
	initResponse, err := kmdClient.InitWalletHandle(exampleWalletID, "testintegration")
	if err != nil {
	...
	signResponse, err := kmdClient.SignTransaction(exampleWalletHandleToken, "testintegration", tx)
	if err != nil {
